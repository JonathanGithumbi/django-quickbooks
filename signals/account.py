...
from django_quickbooks.signals import account_created, account_updated, qbd_task_create

RealmModel = get_realm_model()


@receiver(account_created)
def create_qbd_account(sender, qbd_model_mixin_obj, realm_id, *args, **kwargs):
...
def update_qbd_account(sender, qbd_model_mixin_obj, realm_id, *args, **kwargs):
    qbd_task_create.send(
        sender=qbd_model_mixin_obj.__class__,
