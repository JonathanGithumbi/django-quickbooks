...
from django_quickbooks.signals import vendor_created, vendor_updated, qbd_task_create

RealmModel = get_realm_model()


@receiver(vendor_created)
def create_qbd_vendor(sender, qbd_model_mixin_obj, realm_id, *args, **kwargs):
    qbd_task_create.send(
        sender=qbd_model_mixin_obj.__class__,
        qb_operation=QUICKBOOKS_ENUMS.OPP_ADD,
        qb_resource=QUICKBOOKS_ENUMS.RESOURCE_VENDOR,
...
@receiver(vendor_updated)
def update_qbd_vendor(sender, qbd_model_mixin_obj, realm_id, *args, **kwargs):
    qbd_task_create.send(
        sender=qbd_model_mixin_obj.__class__,
